/* copied from resources/js/web-interface-index.js */
...
window.proceedToCheckout = function() {
	...
	Object.keys(cart).forEach(itemId => {
		...
		checkoutHtml += `<div class="d-flex justify-content-between align-items-center mb-2"><div><strong>${name}</strong><br><small class="text-muted">${qty} x ${price.toLocaleString()} so'm</small></div><strong>${itemTotal.toLocaleString()} so'm</strong></div>`;
	});
...
window.confirmOrder = function() {
	...
	fetch('/api/orders', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }, body: JSON.stringify(orderData) })
	.then(r => r.json()).then(data => {
		...
		} else { if (tg && tg.showAlert) tg.showAlert("Xatolik yuz berdi. Iltimos, qaytadan urinib ko'ring."); }
...
function addPerformanceMonitoring() {
	...
	try { const observer = new PerformanceObserver((list) => { list.getEntries().forEach(entry => { /* no-op */ }); }); observer.observe({ entryTypes: ['resource'] }); } catch (_e) {}
}

function initializeLazyLoading() {
	if (!('IntersectionObserver' in window)) return;
	const imageObserver = new IntersectionObserver((entries, observer) => { entries.forEach(entry => { if (entry.isIntersecting) { const img = entry.target; if (img.dataset.src) { preloadImage(img.dataset.src).then(() => { img.src = img.dataset.src; img.removeAttribute('data-src'); observer.unobserve(img); }).catch(() => { img.src = img.dataset.src; img.removeAttribute('data-src'); observer.unobserve(img); }); } } }); }, { rootMargin: '50px 0px', threshold: 0.1 });
...
function handleImageError(img) {
	...
	const placeholder = img.nextElementSibling; if (placeholder && placeholder.classList.contains('menu-item-image-placeholder')) placeholder.style.display = 'flex';
	const container = img.closest('.image-container'); if (container) { container.classList.remove('loaded'); showImageError(container); addRetryButton(container, img); const fallback = container.querySelector('.menu-item-image-fallback'); if (fallback) fallback.style.display = 'block'; }
}
window.handleImageError = handleImageError;

function addRetryButton(container, img) { let retryBtn = container.querySelector('.retry-button'); if (!retryBtn) { retryBtn = document.createElement('button'); retryBtn.className = 'retry-button'; retryBtn.innerHTML = '<i class="fas fa-redo"></i> Qayta urinish'; retryBtn.onclick = () => retryImageLoad(img); container.appendChild(retryBtn); } retryBtn.style.display = 'block'; }

function retryImageLoad(img) { const container = img.closest('.image-container'); if (container) { container.classList.remove('loaded'); hideImageError(container); const retryBtn = container.querySelector('.retry-button'); if (retryBtn) retryBtn.style.display = 'none'; if (img.dataset.src) img.src = img.dataset.src; else img.src = img.src; } }
...
function handleImageLoad(img) {
	...
	const container = img.closest('.image-container'); if (container) { container.classList.add('loaded'); const fallback = container.querySelector('.menu-item-image-fallback'); if (fallback) fallback.style.display = 'none'; hideImageError(container); hideRetryButton(container); }
}
...
document.addEventListener('visibilitychange', function() { /* no-op */ });

function showImageError(container) { let errorDiv = container.querySelector('.image-error'); if (!errorDiv) { errorDiv = document.createElement('div'); errorDiv.className = 'image-error'; errorDiv.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Rasm yuklanmadi'; container.appendChild(errorDiv); } errorDiv.style.display = 'block'; }
...
function addFocusManagement() { const menuItems = document.querySelectorAll('.menu-item'); menuItems.forEach(item => { item.setAttribute('tabindex', '0'); item.setAttribute('role', 'button'); item.setAttribute('aria-label', `Add ${item.querySelector('.menu-item-title').textContent} to cart`); }); menuItems.forEach(item => { item.addEventListener('focus', function() { this.style.outline = `2px solid ${getComputedStyle(document.documentElement).getPropertyValue('--primary-color')}`; }); item.addEventListener('blur', function() { this.style.outline = 'none'; }); }); }

function addErrorBoundary() { window.addEventListener('error', function() { if (tg && tg.showAlert) tg.showAlert('Xatolik yuz berdi. Iltimos, sahifani yangilang.'); }); window.addEventListener('unhandledrejection', function() { if (tg && tg.showAlert) tg.showAlert('Xatolik yuz berdi. Iltimos, sahifani yangilang.'); }); }

function addOfflineSupport() { window.addEventListener('online', function() { if (tg && tg.showAlert) tg.showAlert('Internet aloqasi tiklandi! 🌐'); }); window.addEventListener('offline', function() { if (tg && tg.showAlert) tg.showAlert("Internet aloqasi yo'q! 📡"); }); }

function addServiceWorkerSupport() { if ('serviceWorker' in navigator) { navigator.serviceWorker.register('/sw.js').catch(() => {}); } }
