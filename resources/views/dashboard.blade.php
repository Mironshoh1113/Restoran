...
@section('content')
<div class="space-y-6">
	@if(isset($subscriptionAlerts) && count($subscriptionAlerts) > 0)
		<div class="bg-red-50 text-red-800 border border-red-200 rounded-xl p-4">
			<ul class="list-disc pl-6">
				@foreach($subscriptionAlerts as $msg)
				<li>{{ $msg }}</li>
				@endforeach
			</ul>
			<a href="{{ route('super.subscriptions.index') }}" class="inline-block mt-2 text-sm text-red-700 underline">Superadmin orqali tarif biriktiring</a>
		</div>
	@endif
	<!-- Welcome Section -->
