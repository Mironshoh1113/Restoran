...
<div class="space-y-4">
	...
	<div class="flex items-center justify-between">
		<div>
			...
			<p class="text-sm text-gray-600">{{ $order->project->name ?? ($order->restaurant->name ?? 'N/A') }}</p>
		</div>
	...
	@php
		$subtotal = $order->orderItems->sum(function($it){ return (float) $it->price * (int) $it->quantity; });
		if ($subtotal == 0 && isset($order->decoded_items)) {
			$subtotal = collect($order->decoded_items)->sum(function($it){ return (float) ($it['price'] ?? 0) * (int) ($it['quantity'] ?? 1); });
		}
		$deliveryFee = (float) ($order->delivery_fee ?? 0);
		$total = $subtotal + $deliveryFee;
		// Fallback to stored total fields if needed
		if ($total == 0) {
			$total = (float) ($order->total_amount ?? $order->total_price ?? 0);
		}
		$restaurant = $order->restaurant ?? ($order->project->restaurant ?? null);
		$allowedPayments = (array) ($restaurant->payment_methods ?? []);
	@endphp
		...
		<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
			...
			<div class="space-y-2 text-sm">
				...
				<div class="flex items-center justify-between">
					...
					<span class="font-medium truncate">{{ Str::limit($order->delivery_address ?? $order->address ?? 'N/A', 25) }}</span>
				</div>
				<div class="flex items-center justify-between">
		...
		<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
			...
			<div class="space-y-2 text-sm">
				<div class="flex items-center justify-between">
					...
					<span class="font-medium">{{ number_format($subtotal, 0, ',', ' ') }} so'm</span>
				</div>
				<div class="flex items-center justify-between">
					<span class="text-gray-600">Yetkazish:</span>
					<span class="font-medium">{{ number_format($deliveryFee, 0, ',', ' ') }} so'm</span>
				</div>
				<div class="flex items-center justify-between">
					<span class="text-gray-600">Jami:</span>
					<span class="text-lg font-bold text-blue-600">{{ number_format($total, 0, ',', ' ') }} so'm</span>
				</div>
				<div class="flex items-center justify-between">
	...
	<!-- Payment controls -->
	<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
		<h3 class="text-base font-semibold text-gray-800 mb-3">To'lov holati</h3>
		<form action="{{ route('admin.orders.update-payment', $order) }}" method="POST" class="flex flex-col sm:flex-row sm:items-center sm:space-x-3 space-y-2 sm:space-y-0">
			@csrf
			@method('PATCH')
			<div class="flex items-center space-x-2">
				<label class="text-sm text-gray-700">Holat:</label>
				<select name="is_paid" class="px-2 py-1 border border-gray-300 rounded text-sm">
					<option value="0" {{ !$order->is_paid ? 'selected' : '' }}>To'lanmagan</option>
					<option value="1" {{ $order->is_paid ? 'selected' : '' }}>To'langan</option>
				</select>
			</div>
			<div class="flex items-center space-x-2">
				<label class="text-sm text-gray-700">Usul:</label>
				<select name="payment_method" class="px-2 py-1 border border-gray-300 rounded text-sm">
					...
					@else
						@foreach($allowedPayments as $pm)
							<option value="{{ $pm }}" {{ ($order->payment_method ?? $order->payment_type) === $pm ? 'selected' : '' }}>{{ ucfirst($pm) }}</option>
						@endforeach
					@endif
				</select>
			</div>
			<button type="submit" class="px-3 py-1 bg-green-600 hover:bg-green-700 text-white rounded text-sm">Saqlash</button>
		</form>
	</div>
	...
	@if($order->status !== 'delivered' && $order->status !== 'cancelled')
		<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
			...
			@php
				$availableCouriers = \App\Models\Courier::where('restaurant_id', $order->project->restaurant_id ?? $order->restaurant_id ?? null)->get();
			@endphp
